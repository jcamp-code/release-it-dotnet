...
import { DefaultUpdater, type UpdateOptions } from './default'
  ...
  updateContent(content: string | undefined): string {
    ...
    content.split(/\r?\n/).forEach((line) => {
      ...
      if (match) {
        ...
        if (line.match(this.blockEndRegex)) blockScope = undefined
      } else {
        // look for block start line
        match = line.match(this.blockStartRegex)
        if (match)
          if (match.groups?.scope) blockScope = match.groups.scope as BlockScope
          else blockScope = 'version'
